...
There are a few key pieces of information here that show us that females in our sample are *significantly* taller than females in the population. In formal language, these results suggest we can *reject the null hypothesis that the national mean and the sample mean could be the same*. 
...
The mean is only one summary statistic. IUn some cases, only looking at the mean will obscure more interesting insights. Whenever possible, it is preferable to look at the full distribution of values.
